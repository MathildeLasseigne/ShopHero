...
public class MidiGameController : MonoBehaviour
{
    ...
    private bool tempoChanged = false;
    private double changedTempo = 120;

    [SerializeField, Tooltip("Optional")] private MidiValueFilterScriptable noteFilter;
    ...
    void Start()
    {
        if (midiFilePlayer != null)
        {
            ...
            midiFilePlayer.OnEventStartPlayMidi.AddListener(info => OnPlayMidi());
    ...
    public void SetMidiTempo(double tempo)
    {
        tempoChanged = true;
        changedTempo = tempo;
    }
    ...
    /// </summary>
    /// <param name="startDelayMilli"></param>
    ...
    private void OnPlayMidi()
    {
        InputMidiStartPositionMilli();
        SetTempoForMidi();
    }

    private void SetTempoForMidi()
    {
        if(tempoChanged)
            midiFilePlayer.MPTK_Tempo = changedTempo;
    }
    ...
    public void FilterNotePlayedMPTKEvent(List<MPTKEvent> notes)
    {
        ...
        bool isValid = true;
        foreach (MPTKEvent mptkEvent in notes) // Doubles or triple notes. All in the same tick. Count as 1
        {
            if(mptkEvent != null && mptkEvent.Command == MPTKCommand.NoteOn)
            {
                ...
                isNoteOn = true;
                isValid &= noteFilter != null? noteFilter.isValid(mptkEvent) : true;
